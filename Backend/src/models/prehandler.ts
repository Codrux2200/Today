...
export const VerifyToken = async (req: FastifyRequest<{ Params: any }>, reply: FastifyReply) => {
    try {
  ...
  export const VerifyAdminToken = async (req: FastifyRequest<{ Querystring: any }>, reply: FastifyReply) => {
    try {
      // Récupérer le token depuis l'en-tête Authorization (format: "Bearer <token>")
      const token = req.headers['authorization']?.split(' ')[1]; // Prendre la partie après "Bearer"
      console.log(token);
      ...
      // Vérifier et décoder le token
      const secretKey = process.env.JWT_ADMIN_SECRET || 'your_jwt_secret'; // Votre clé secrète
      const decoded: any = jwt.verify(token, secretKey); // Décodage du token
      if (!decoded || !decoded.id) {
        return reply.status(401).send({ message: 'Invalid token' });
      }
